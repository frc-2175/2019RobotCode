...
public class VisionSubsystem {
	...
	public static final double LIMELIGHT_HEIGHT_FROM_GROUND = 9;
	...
	public static final double CAMERA_ROTATION_Y = 25;
	 ...
	 */
	public Vector getPositionOfPoint(Vector normalizedCameraPosition) {
	 ...
	 * @return the position of the pixel relative to the camera in a top-down view
	 */
	public Vector getPositionOfPoint(double pixelX, double pixelY) {
		return getPositionOfPoint(convertFromPixelsToNormalized(new Vector(pixelX, pixelY)));
	}
	 ...
	 */
	public Vector getPositionOfPointRobotSpace(Vector normalizedCameraPosition) {
		Vector positionCameraSpace = getPositionOfPoint(normalizedCameraPosition);
		Vector positionRobotSpace = positionCameraSpace.rotate(CAMERA_ROTATION_Z);
		SmartDashboard.putNumber("AutoPopulate/TargetposX", positionRobotSpace.x);
		return positionRobotSpace = positionRobotSpace.add(CAMERA_ROBOT_SPACE);
	}
	 ...
	 * @return the position of the pixel relative to the robot in a top-down view
	 */
	public Vector getPositionOfPointRobotSpace(double pixelX, double pixelY) {
		return getPositionOfPointRobotSpace(convertFromPixelsToNormalized(new Vector(pixelX, pixelY)));
	}
	 ...
	 */
	public Vector[] getPositionOfCorners() {
		...
		if(corners.length >= 8) {
			...
			Vector leftTargetPosition = getPositionOfPoint(leftTarget.x, leftTarget.y);
			Vector rightTargetPosition = getPositionOfPoint(rightTarget.x, rightTarget.y);
	...
	public double getTargetZRotation() {
		Vector[] corners = getPositionOfCorners();
	...
	public Vector[] genPathToTargetCargo(int resolution) {
		...
		double targetZRotation = getTargetZRotation();
		bezier = new Bezier(targetLocation.x, targetLocation.y, PATH_STRENGTH, targetZRotation);
